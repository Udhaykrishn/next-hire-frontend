import { AlertTriangle, ArrowRight, CheckCircle, Loader2, ShieldCheck } from "lucide-react";
import { motion } from "motion/react";
import Link from "next/link";
import { Button } from "@/components/animate-ui/components/buttons/button";
import { JobWithMatchScore, CandidateApplicationResponse } from "@/features/jobs/types/job.types";
          ...
          {hasApplied ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-3 py-1.5 text-[11px] font-black uppercase rounded-full border ${getStatusColor(applicationStatus)}`}>
              ...
              <p className="text-[14px] text-gray-500 font-medium pb-6 border-b border-gray-100 leading-relaxed">
                You submitted your application <strong className="text-gray-900 font-bold">{formatDaysAgo(existingApplication?.application?.createdAt)}</strong>. We will notify you when the employer updates your status.
              </p>
              
              <Link href="/applications" className="w-full h-14 rounded-2xl text-[15px] font-black transition-all flex items-center justify-center gap-2 bg-gray-50 text-gray-700 hover:bg-gray-100 border border-gray-200 shadow-sm">
            ...
            <>
              ...
              <Button
                ...
                className={`w-full h-14 rounded-2xl text-[15px] font-black transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2 ${job.status !== "OPEN"
      ...
      <div className="bg-white rounded-[2.5rem] p-8 border border-wise-green/10 shadow-lg shadow-wise-green/5 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-[13px] text-gray-600 font-medium leading-relaxed mb-6">
            <strong className="text-dark-green">{job.hiringCompany}</strong> has a verified presence on Next Hire.
          </p>
        </div>
      </div>
    </aside>
