...
export function AIMatchAnalysis({ matchScore, matchBreakdown, isAnalyzing, onAnalyze }: AIMatchAnalysisProps) {
  return (
    <div className="bg-[#ffffff] p-6 rounded-[20px] border border-[#38c8ff]/30 shadow-sm space-y-4 relative overflow-hidden group">
      ...
      {matchScore === null ? (
        <div className="flex flex-col items-center justify-center py-4 relative z-10">
          <p className="text-[13px] text-[#868685] text-center mb-4">Analyze the candidate's resume, skills, and experience against the job requirements.</p>
          <button 
            onClick={() => onAnalyze()}
          ...
          >
            {isAnalyzing ? (
               ...
               <>
                 <Sparkles className="w-4 h-4 text-[#38c8ff]" /> Calculate Match Score
               </>
            )}
          </button>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-2 space-y-3 relative z-10">
          <div className="relative flex items-center justify-center w-28 h-28">
            <svg className="w-full h-full transform -rotate-90">
              <circle cx="56" cy="56" r="48" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-[#f4f6f3]" />
              <circle 
                cx="56" 
                cy="56" 
                r="48" 
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[26px] font-[800] text-[#0e0f0c]">{matchScore}%</span>
            </div>
          </div>
          <p className="text-[14px] font-[700] text-[#0e0f0c]">
            {matchScore >= 80 ? "Excellent Match" : matchScore >= 60 ? "Good Match" : matchScore >= 40 ? "Fair Match" : "Poor Match"}
          </p>
          <div className="flex gap-2 mt-2 w-full">
            <button onClick={() => onAnalyze(true)} className="flex-1 text-[12px] font-[600] text-[#868685] hover:text-[#054d28] bg-[#f9faf9] border border-[rgba(14,15,12,0.05)] py-1.5 rounded-lg transition-colors">
              Re-analyze
            </button>
          </div>
          ...
          {matchBreakdown && (
            <div className="mt-4 w-full text-left space-y-3 bg-[#f4f6f3] p-4 rounded-[12px] border border-[rgba(14,15,12,0.05)]">
              <div>
                <p className="text-[12px] font-[600] text-[#868685] uppercase tracking-wider mb-1.5">AI Notes</p>
                <p className="text-[13px] font-[500] text-[#454745] leading-relaxed">{matchBreakdown.notes}</p>
              </div>
              {matchBreakdown.keywords && matchBreakdown.keywords.length > 0 && (
                <div>
                  <p className="text-[12px] font-[600] text-[#868685] uppercase tracking-wider mb-2">Matched Keywords</p>
                  <div className="flex flex-wrap gap-1.5">
                    {matchBreakdown.keywords.map((kw, i) => (
                      <span key={i} className="text-[11px] font-[600] text-[#054d28] bg-[#054d28]/10 px-2 py-0.5 rounded-full border border-[#054d28]/20">
                        {kw}
                      </span>
                    ))}
                  </div>
