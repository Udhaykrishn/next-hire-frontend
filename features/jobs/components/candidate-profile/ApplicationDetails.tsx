...
const getStatusStyles = (status: string) => {
  switch (status) {
    ...
    case "SHORTLISTED": return "bg-[#cdffad] text-[#054d28] border-[#9fe870]/30";
...
export function ApplicationDetails({ status, createdAt, resumeUrl }: ApplicationDetailsProps) {
  return (
    <div className="bg-[#ffffff] p-6 rounded-[20px] border border-[rgba(14,15,12,0.08)] shadow-sm space-y-5">
      <h3 className="text-[16px] font-[800] text-[#0e0f0c] border-b border-[rgba(14,15,12,0.08)] pb-3">Application Details</h3>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <p className="text-[12px] font-[600] text-[#868685] uppercase tracking-wider mb-1.5">Status</p>
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-[13px] font-[700] border ${getStatusStyles(status)}`}>
            {status}
          </span>
        </div>
        <div>
          <p className="text-[12px] font-[600] text-[#868685] uppercase tracking-wider mb-1.5">Applied On</p>
