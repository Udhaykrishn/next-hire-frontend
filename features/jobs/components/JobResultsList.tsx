          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10 pb-6">
              <button
                ...
                disabled={page === 1}
                className="px-4 py-2 rounded-xl text-[14px] font-bold border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                Previous
              </button>
              <div className="flex items-center gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                  (p) => {
                    // Only show current page, 1 page before, 1 page after, first, and last (simple pagination)
                    if (
                      p === 1 ||
                      p === totalPages ||
                      (p >= page - 1 && p <= page + 1)
                      ...
                      return (
                        <button
                          ...
                          className={`w-10 h-10 rounded-xl text-[14px] font-bold transition-all ${
                            page === p
                              ? "bg-wise-green text-dark-green border-2 border-wise-green shadow-sm"
                    ...
                    if (p === page - 2 || p === page + 2) {
                      return (
                        <span key={p} className="text-gray-400 font-bold px-1">
                          ...
                        </span>
                      );
                    }
