...
export function useBulkUpdateApplicationStatusMutation(jobId: string) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data } = await apiClient.patch(`/job/application/bulk/status`, {
        applicationIds,
        status,
      });
      return data;
    ...
    onSuccess: (_, variables) => {
      toast.success(
        `Successfully updated ${variables.applicationIds.length} candidates to ${variables.status.replace("_", " ")}`,
      );
      queryClient.invalidateQueries({
        queryKey: ["recruiter-job-applications", jobId],
      });
    ...
    onError: (error) => {
      toast.error("Failed to bulk update application statuses");
      console.error(error);
    },
  });
}
