...
import { Check, ChevronDown, Plus, Building, Settings } from "lucide-react";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger className="flex items-center gap-2 py-1.5 px-3 rounded-lg border border-gray-100 bg-gray-50/30 hover:bg-gray-100/50 transition-colors focus:outline-none">
				<div className="w-6 h-6 rounded-md bg-near-black flex items-center justify-center text-white overflow-hidden">
			...
			<DropdownMenuContent align="end" className="w-64">
				{companies.map((company) => (
					...
					>
						<div className="flex items-center gap-2">
							<div className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center overflow-hidden">
								{company.logo_url ? (
							...
							<span className={cn("text-[13px] font-medium", activeCompany?._id === company._id ? "text-near-black font-bold" : "text-gray-600")}>
								{company.name}
							</span>
						</div>
				...
				>
					...
					<span className="text-[13px] font-bold text-near-black">Manage Companies</span>
				</DropdownMenuItem>
			</DropdownMenuContent>
		</DropdownMenu>
	);
