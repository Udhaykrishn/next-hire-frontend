...
const normalizeRound = (raw: unknown): InterviewRound => {
  ...
  return {
    ...
    interviewerIds: (pick("interviewerIds") ?? []) as string[],
    interviewers: pick("interviewers") as InterviewRound["interviewers"],
    templateId: pick("templateId") as InterviewRound["templateId"],
    title: (pick("title") ?? "") as string,
    type: (pick("type") ?? "VIDEO") as string,
    timeZone: (pick("timeZone") ?? "") as string,
    instructions: pick("instructions") as string | undefined,
    internalNotes: pick("internalNotes") as string | undefined,
