...
export function HeroAvatarCard({
  ...
  activeCompany: CompanyProfileData | null;
  isUploadingAvatar: boolean;
  handleUploadAvatar: (file: File) => Promise<void>;
  isDeletingAvatar: boolean;
  handleDeleteAvatar: () => Promise<void>;
}) {
  const avatarUrl = activeCompany?.logo || recruiterProfile?.profile_url?.url || "";
  const displayName = activeCompany?.name || formData.name || "Your Company";
  const displayRole = activeCompany?.industry || formData.companyRole || "Industry";
