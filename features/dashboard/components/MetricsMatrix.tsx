...
export function MetricsMatrix({
  ...
  matrix: DashboardMetrics["matrix"];
}) {
  const [activeTab, setActiveTab] = useState<
    "overview" | "jobs" | "candidates" | "tasks"
  >("overview");

  return (
    <>
      <div className="border-b-2 border-near-black mb-8 flex gap-8 overflow-x-auto no-scrollbar">
        {[{ id: "overview", label: "Data Overview" }].map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {activeTab === "overview" && (
          <>
            {matrix.recruitmentStatus && (
              <MetricsList
                title="Recruitment Status"
                data={matrix.recruitmentStatus}
              />
            )}
            {matrix.candidateFunnel && (
              <MetricsList
                title="Candidate Funnel"
                data={matrix.candidateFunnel}
              />
            )}
            {matrix.interviewAnalytics && (
              <MetricsList
                title="Interview Analytics"
                data={matrix.interviewAnalytics}
              />
            )}
          </>
        )}
