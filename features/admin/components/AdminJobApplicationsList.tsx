...
import { Users, FileText, ExternalLink, Search, Filter } from "lucide-react";
...
export const AdminJobApplicationsList = ({ jobId }: { jobId: string }) => {
  ...
  const { data, isLoading } = useAdminJobApplications(jobId, page, limit, debouncedSearch, statusFilter);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-5 mb-5">
        <div className="flex items-center gap-3">
          ...
          <h4 className="font-black text-lg text-near-black tracking-tight">
            Recent Applications {data?.total !== undefined ? `(${data.total})` : ''}
          </h4>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-4 text-xs font-bold text-gray-400 uppercase tracking-widest px-4">Candidate</th>
                  <th className="pb-4 text-xs font-bold text-gray-400 uppercase tracking-widest px-4">Applied Date</th>
                  <th className="pb-4 text-xs font-bold text-gray-400 uppercase tracking-widest px-4">Status</th>
                  <th className="pb-4 text-xs font-bold text-gray-400 uppercase tracking-widest px-4 text-right">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr key={app.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h5 className="font-bold text-near-black text-sm">{app.candidate.name}</h5>
                          <p className="text-xs text-gray-500 font-medium">{app.candidate.email}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest ${app.status === 'HIRED' ? 'bg-green-100 text-green-700' :
