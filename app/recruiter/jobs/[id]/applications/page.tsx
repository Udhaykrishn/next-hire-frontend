...
export default function JobApplicationsPage() {
  ...
  const { data: jobData, isLoading: isJobLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<JobDetailResponse>(`/job/${id}`);
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<JobStatsResponse>(
        `/job/${id}/stats`,
      );
      return response.data;
    },
                  ...
                  <div className="flex justify-between items-start mb-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col items-start">
                        <button
                          type="button"
                          className="font-[700] text-[16px] text-[#0e0f0c] hover:text-[#054d28] transition-colors cursor-pointer text-left bg-transparent border-0 p-0 focus:outline-none focus:underline"
                          onClick={() =>
                            router.push(
