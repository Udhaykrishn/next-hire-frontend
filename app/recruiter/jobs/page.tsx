...
import { JobResponse } from "@/features/jobs/types/job.types";
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-near-black tracking-tight">Your Jobs</h1>
          <p className="text-slate-500 mt-2 font-medium">Manage your job postings and view applications.</p>
        </div>
        <Link 
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {jobs.map((job: JobResponse) => (
            <div key={job.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-wise-green/50 transition-colors">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h3 className="text-lg font-bold text-near-black">{job.jobTitle}</h3>
                    <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-black rounded-full ${job.status === 'OPEN' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                      {job.status}
                    </span>
                    {!job.is_published && (
                  ...
                  <div className="flex items-center gap-4 mt-3 text-xs text-slate-400 font-medium">
                    <span>Posted {format(new Date(job.created_at || Date.now()), "MMM d, yyyy")}</span>
                    <span>•</span>
                    <span className="text-wise-green/80 font-bold">0 Applications</span>
                  </div>
                </div>
              </div>
              
              <div className="flex items-center gap-3">
                <Link href={`/recruiter/jobs/edit/${job.id}`} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-near-black transition-colors rounded-full hover:bg-slate-50 border border-transparent hover:border-slate-200">
                  Edit Job
                </Link>
        ...
        <div className="text-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
          ...
          <h3 className="text-xl font-bold text-near-black mb-2">No jobs posted yet</h3>
          <p className="text-slate-500 mb-6 max-w-sm mx-auto">You haven't created any job listings. Post your first job to start receiving applications.</p>
          <Link 
            href="/recruiter/jobs/create/new"
